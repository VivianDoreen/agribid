...
def add_farmer_produce(current_user):
    ...
    create_farmer_produce = ProduceModel(
        current_user, data['produce_name'], data['quantity'], data['unit_price'])
...
def get_products():
    """
    This endpoint gets all farmer produce list
    :return: 
    """
    farmer_produce_list = ProduceModel.get_farmer_produce()
    return jsonify({'farmerProduce': farmer_produce_list}), 200
