...
class ProduceModel(object):
    ...
    def create_produce(self):
        ...
        try:
            ...
            connection.cursor.execute(query_to_add_produce, (self.produce_name, self.quantity,
                                                             self.unit_price, self.current_user, self.date_created, self.date_modified))
            query_to_search_product = "SELECT * FROM farmer_produce WHERE produce_name=%s"
            connection.cursor.execute(
                query_to_search_product, [self.produce_name])
